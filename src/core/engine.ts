...
export class OboldEngine extends EventEmitter {
  ...
  private async evaluateReminders(sw: SwitchConfig, nextDeadline: number, now: number): Promise<void> {
    ...
    for (const rem of sw.reminders) {
      ...
      if (sentSet.has(rem.id) || metaStatus === 'SENT') {
        sentSet.add(rem.id);
        continue;
      }
      if (metaStatus === 'IN_FLIGHT') {
        const pendingEntries = this.db.getPendingOrRetryableLedgerEntries();
        const activeEntry = pendingEntries.find(
          (e) => e.switchId === sw.id && e.stageId === 'reminder' && e.actionId === rem.id
        );
        if (activeEntry && (activeEntry.state === 'EXECUTING' || activeEntry.state === 'CLAIMED')) {
          continue;
        }
      }
