...
export function generateRescueBundle(options: RescueBundleOptions): RescueBundleResult {
  ...
  const decoderScript = `
    ...
    async function combineShamirShares(shares) {
      const parsed = [];
      const seenX = new Set();
      let expectedK = 0;
      ...
      for (const shareStr of shares) {
        ...
        let t, xVal, hexData;
        if (v1Match) {
          const setId = v1Match[1];
          t = parseInt(v1Match[2], 10);
          const totalN = parseInt(v1Match[3], 10);
          xVal = parseInt(v1Match[4], 10);
          hexData = v1Match[5];
          const checksum = v1Match[6];

          const expectedHash = await sha256Hex(setId + ':' + t + ':' + totalN + ':' + xVal + ':' + hexData);
          const expectedChecksum = expectedHash.substring(0, 8);

          if (checksum.toLowerCase() !== expectedChecksum.toLowerCase()) {
            throw new Error('Checksum mismatch on share x=' + xVal + '. Share may be corrupted or tampered.');
          }
    ...
    document.getElementById('decrypt-btn').addEventListener('click', async () => {
      ...
      try {
        const reconstructedKeyHex = await combineShamirShares(rawShares);
...
<body>
  <div class="vault-card">
    ...
    <div class="footer">
      Generated by obold v${DAEMON_VERSION} &bull; Bundle ID: ${bundleId} &bull; Self-contained offline HTML vault &bull; Zero network egress required
    </div>
  </div>
