...
async function terminateProcessTree(proc: any, isWindows: boolean): Promise<void> {
  ...
  if (proc.pid) {
    try {
      if (isWindows) {
        const killer = Bun.spawn(['taskkill', '/pid', proc.pid.toString(), '/T', '/F'], {
          stdout: 'ignore',
          stderr: 'ignore',
        });
        await Promise.race([killer.exited, new Promise((r) => setTimeout(r, 1000))]);
      } else {
        try {
          process.kill(-proc.pid, 'SIGKILL');
        } catch {}
        try {
          Bun.spawn(['pkill', '-9', '-P', proc.pid.toString()], { stdout: 'ignore', stderr: 'ignore' });
        } catch {}
        try {
          process.kill(proc.pid, 'SIGKILL');
        } catch {}
...
export class ShellPlugin implements IPluginExecutor {
  ...
  async execute(config: Record<string, any>, context: PluginExecutionContext): Promise<ExecutionResult> {
    ...
    try {
      ...
      if ((context.signal && context.signal.aborted) || (outcome && outcome.isAbort)) {
        await terminateProcessTree(proc, isWindows);
        return {
          success: false,
          actionId: context.actionId,
          plugin: this.id,
      ...
      if (timedOut || (outcome && outcome.isTimeout)) {
        await terminateProcessTree(proc, isWindows);
        return {
          success: false,
          actionId: context.actionId,
